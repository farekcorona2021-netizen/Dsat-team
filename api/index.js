...
const akwam = require('../lib/providers/akwam');
const arabseed = require('../lib/providers/arabseed');

const PROVIDERS = [faselhd, egydead, akwam, arabseed];
...
const manifest = {
  ...
  version: '1.0.0',
  name: 'Arabic Streams',
  description: 'Arabic movies and series from FaselHD, EgyDead, Akwam, and ArabSeed',
...
builder.defineMetaHandler(async (args) => {
  ...
  // For meta, we can return a basic structure since we already have the data from search
  // In a real implementation, you'd fetch the specific meta page
  return {
    meta: {
      id,
...
builder.defineStreamHandler(async (args) => {
  ...
  if (parsed.type === 'movie') {
    ...
    // For series, we need season and episode from the ID or args
    // Stremio passes series IDs as series:tt123:1:2 (series:imdb:season:episode)
    // But our prefixed IDs are provider:series:url
    // We need to handle this differently - perhaps store season/episode in the URL or use args
    
    // Default to season 1 episode 1 if not specified
    const season = 1;
    const episode = 1;
    streams = await provider.getSeriesStreams(parsed.contentId, season, episode);
  }
);
  }
  ...
  <div class="container">
    <div class="hero">
      ...
      <div class="providers">
        ...
        <span class="provider-badge">🍿 Akwam</span>
        <span class="provider-badge">🎞️ ArabSeed</span>
      </div>
    ...
    <div class="features">
      <div class="feature-card">
        ...
        <div class="feature-desc">ابحث في جميع المصادر العربية من مكان واحد مع دعم كامل للغة العربية</div>
      </div>
      <div class="feature-card">
        <div class="feature-icon">⚡</div>
