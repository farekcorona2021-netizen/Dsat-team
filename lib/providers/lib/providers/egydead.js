...
const BASE_URL = 'https://egydead.fyi';
const PROVIDER_ID = 'egydead';
...
async function search(query) {
  ...
  try {
    ...
    $('.Grid--MycimaPosts > li, .MovieBlock').each((_, el) => {
      ...
      const yearText = $el.find('.year, .num, .number').text() || '';
...
async function getMovieStreams(id) {
  ...
  try {
    ...
    $('video source, .player source, source').each((_, el) => {
    ...
    for (const url of urlMatches) {
      ...
      streams.push({
        name: `EgyDead ${quality}`,
        title: `${quality}`,
        url: normalized,
        behaviorHints: {
          notWebReady: normalized.endsWith('.m3u8'),
          bingeGroup: `egydead-${quality}`,
        },
      });
    }
